...
- Inherited, not introduced here: Role 1's LLM reads raw patient note text via RAG, and that text flows unsanitized into `MultiAgentAnswer.citations` (via `Citation.snippet`, §2 above) — never into `MultiAgentAnswer.answer` or any LLM prompt in Block 5 or Block 6's current code (traced: Block 5's answer-writing call receives only patient IDs and drug counts; Block 6's reconciliation is pure string templating, no LLM call). This is an indirect-prompt-injection surface (OWASP LLM Top 10) that predates Block 6 — dormant today, not live, but worth a paper trail here so Block 7 doesn't discover it cold, and becomes live the moment citations are fed back into a model or rendered raw.
...
- Automated eval suite runs in CI: reuse Block 5's fixed question set — originally 11 questions total, 8 scored for recall (re-measuring the 2 previously-capped questions; target full recall now that the cohort agent is unbounded), plus 3 deliberately unanswerable control questions (Block 5's `answerable: false` entries) checked pass/fail that the system correctly reports no matches rather than scored for recall. Extended to 12 total / 9 scored in Phase 5 (q12, see `docs/plan.md` §10) to cover a real cohort exceeding the old top_k=25 ceiling with a non-zero drug count — no combination in Block 5's original set exercised that path. Add ≥1 new eval dimension exercising the failure matrix in §4 (inject synthetic tool failures via fakes, assert correct `mode` and no unhandled exception).
