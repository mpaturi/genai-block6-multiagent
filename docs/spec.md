...
- **Verify the cost-per-token rate.** `scripts/run_log.py`'s cost accounting inherits its $/token constants from Block 5's `logging_utils.py`, which documents itself as "not the source of truth for pricing." Phase 5 surfaced this concretely: every `cost_usd` figure this system has reported so far (including in the README) carries that same unverified-rate caveat forward. Confirm against Anthropic's current published pricing before treating any `cost_usd` number this system produces as a real budget figure.
