...
**Gap found during Phase 1 interface re-verification (2026-07-24), not previously known:** `ClinicalAnswer` carries no snippet/source text anywhere — `graph_result` is drug counts (`{drug_name: count}`), not evidence text, and Block 4's `/query` response strips `chunk_text` before returning `sources` (only `person_id`/`chunk_id`/`score` cross the API boundary; confirmed by reading `genai-block4-rag-eval/scripts/api.py` and `retrieve.py`). So `rag_patient_ids` is the only real citation data Role 1 currently exposes. Block 6's own `Citation.snippet: str` (§3) has no data source yet under this reality — resolve in Phase 2/3: either populate `Citation.snippet` from a Block 4/5 change (out of scope per §8 — not this block's job) or drop/repurpose the `snippet` field to reflect what's actually available (e.g. omit it, or state plainly it's unpopulated with a fixed placeholder). Do not implement Phase 2's `Citation` schema against the field list in §3 without resolving this first.
...
- `Citation`: `patient_id: int`, `snippet: str`, `source: str` — one cited piece of evidence, not a bare string. **Unresolved as of the Phase 1 interface re-check (§2's Role 1 note):** Role 1's `ClinicalAnswer` has no data source for `snippet` today. Do not implement this field's population in Phase 2 without first deciding, in plan.md, what it actually gets populated with.
...
- Block 5 (`genai-block5-agent`): `run_agent`, `ClinicalAnswer`, `QuestionInput`. **Re-verified 2026-07-24 against the live repo:** signatures and field lists match this spec's description exactly — `run_agent(question, *, search_fn=search_patients, count_fn=count_drugs, answer_fn=_default_answer_fn) -> tuple[ClinicalAnswer, bool]`, `top_k=25` (confirmed still 25, not reverted to 20), confidence tiers `low <15 / medium 15–24 / high ≥25` unchanged. One gap found in the same pass — see Role 1's note above (Citation.snippet has no data source).
- Block 4 (`genai-block4-rag-eval`): called transitively via Block 5's `search_fn`, not directly by this repo. **Re-verified 2026-07-24:** `SEARCH_TIMEOUT_SECONDS = 10` (`retrieve.py:69`) and `GENERATE_TIMEOUT_SECONDS = 10` (`generate.py:31`) confirmed current; both wrapped in one `except Exception` in `api.py`'s `/query` handler that returns a generic 502 — confirmed accurate, matches this spec's claim.
- Block 5's own outbound HTTP client to Block 4's `/query` (previously unverified — flagged in the original plan.md §7 caveat and tasks.md Phase 1): **confirmed 2026-07-24**, `rag_tool.py:60` sets `requests.post(..., timeout=10)`. This is *tighter* than the ~20s/attempt figure plan.md §7 built from Block 4's internal 10s+10s ceiling alone, not looser — the client aborts at 10s regardless of what Block 4 might still be doing server-side. See `docs/plan.md` §7 for the corrected worst-case math this produces.
- Block 3 (`genai-block3-graph-kb`): Neo4j instance/schema the Cohort Agent queries directly via Cypher. **Re-verified 2026-07-24:** `HAS_CONDITION`/`PRESCRIBED` relationship names and `condition_name` property confirmed current in `query_graph.py`; `NEO4J_URI`/`NEO4J_USER`/`NEO4J_PASSWORD`/`NEO4J_DATABASE` env-var convention confirmed current in `check_connection.py`. Driver setup and relationship-name conventions confirmed reusable (see §2).
