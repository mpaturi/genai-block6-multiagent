...
- **Branch-level supervisory timeout:** derived from Role 1's own worst case, since it's the slower agent. Confirmed constants: `_MAX_TOOL_RETRIES = 2` (3 attempts) on search (RAG, up to 10s Pinecone + 10s Claude per attempt *inside Block 4's own `/query` handler* ⇒ up to ~20s/attempt ⇒ ~60s worst case for 3 attempts), 3 attempts on count (10s Neo4j timeout each ⇒ ~30s worst case), and `_MAX_ANSWER_RETRIES = 1` (2 attempts) on answer synthesis (assume comparable ~10s/attempt ⇒ ~20s worst case). Sum: **~110s theoretical worst case for Role 1** if every retry is exhausted.

  **One caveat on the search number specifically, flagged rather than silently assumed:** the ~20s/attempt figure is built entirely from Block 4's *internal* Pinecone/Claude timeouts, both independently confirmed by direct code read. It assumes Block 5's own outbound HTTP client (calling Block 4's `/query` endpoint from `rag_tool.py`) has no tighter or looser timeout of its own that would change this — that client-side timeout value was never actually checked in any audit so far. Re-verify it in Phase 1's interface re-check (`tasks.md`) alongside the other Block 5 interface confirmations, rather than trusting this number as fully "confirmed" the way the Neo4j 10s figure is.

  Setting the branch-level ceiling below the ~110s worst case would cut off legitimate retries the tool's own logic is still working through — so the ceiling exists only to catch hangs *outside* what these timeouts already cover (e.g. a hung DNS resolution or TCP handshake before a client library's own timeout clock even starts). **Decision: 150s per branch**, applied uniformly to both (Role 2's own worst case is much shorter — 3 attempts × 10s ⇒ ~30s — so 150s is generous headroom for it too, not a tight fit; if Phase 1's re-check finds the search number needs revising, revisit 150s too, don't leave it orphaned from its own justification). Enforced via `asyncio.wait_for(...)` wrapping each `loop.run_in_executor(block6_executor, ...)` call in §4/§5's node wrappers; a `TimeoutError` here is caught by §5's node-level try/except and classified as `"timeout"` — with the caveat from §4's thread-leak note that this stops the wait, not the underlying thread.
...
The eval harness runs against the same frozen `ci_graph_seed.cypher` snapshot Block 5's CI already loads (`data/seed/ci_graph_seed.cypher`, copied from Block 5's current file, not regenerated) — loaded fresh into a disposable Neo4j instance per CI run (same seed-and-reset pattern Block 5 established), not a live or shared graph. This directly addresses the "moving graph" risk spec §6 flagged: the enumeration query's result is only meaningful if the population it's enumerating is fixed and known.
...
Flagging as an assumption to confirm, not asserting it: this project's patient data is treated as synthetic/de-identified for the purposes of this portfolio exercise. On that assumption, full state (including patient IDs and RAG-cited snippets) is accepted as-is in LangSmith traces for this block — no redaction implemented now. **If that assumption is wrong** (i.e. this dataset is or resembles real PHI), this decision needs to be revisited before any trace data leaves this machine.

This can't just live as prose here — `tasks.md` Phase 1 has an explicit, gating task requiring this to be confirmed with you directly before Phase 5 (the first phase that produces real trace data against live infrastructure). Work should not proceed past that gate on the assumption above without your explicit confirmation.
