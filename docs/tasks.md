...
- [x] Re-verify Block 3/4/5's current interfaces against `docs/spec.md` §5 before moving to Phase 2. Done 2026-07-24: `run_agent`'s signature, `ClinicalAnswer`'s fields, `QuestionInput`'s fields, `top_k=25`, and confidence tiers all confirmed unchanged. Block 4's `/query` contract and both 10s timeouts confirmed unchanged. Block 3's relationship names/env-var convention confirmed unchanged. **One new gap found, not previously known:** `ClinicalAnswer` carries no snippet/citation text anywhere (`graph_result` is drug counts, not evidence text; Block 4's `/query` strips `chunk_text` before returning `sources`) — Block 6's spec'd `Citation.snippet` field has no current data source. Flagged in `docs/spec.md` §2 and must be resolved before Phase 2 implements the `Citation` schema.
- [x] **Check the one unverified number in plan.md §7:** what timeout does Block 5's `rag_tool.py` actually set on its own outbound HTTP call to Block 4's `/query` endpoint? Confirmed 2026-07-24: `timeout=10` (`rag_tool.py:60`) — tighter than the assumed 20s/attempt, not looser. §7's worst-case math corrected: ~80s theoretical worst case for Role 1 (was ~110s). The 150s branch timeout is left unchanged — already generous headroom under the original estimate, now more so under the corrected one.
...
- [x] **Blocking — confirm before Phase 5:** is the patient data used across Blocks 3–6 synthetic/de-identified, or does it resemble real PHI? (plan.md §16). Confirmed by Mili, 2026-07-24: patient data across Blocks 3–6 is synthetic/de-identified, not real PHI. Phase 5 is clear to trace full state to LangSmith as-is — see plan.md §16.
- [ ] **Decide in plan.md, before Phase 2 implements `scripts/schemas.py`:** what populates `Citation.snippet` given Role 1 has no snippet-text data source (found during interface re-check above; flagged in `docs/spec.md` §3). Options to weigh: drop the field, populate it with a fixed placeholder, or propose a Block 4/5 change in *their* repos (spec §8 — not doable here) to start returning `chunk_text` per source.
