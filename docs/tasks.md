...
- [x] **`Citation.snippet`'s data source, resolved 2026-07-24:** Block 5's `ClinicalAnswer.rag_citations` (`{patient_id, chunk_id, snippet}`, via `build_rag_citations()`) is the confirmed source — no placeholder or Block 4/5 change needed. `docs/spec.md` §2/§3 updated. Phase 2 writes `test_schemas.py`/`test_orchestrator.py` `Citation` cases against this real field, not a stub.
